...
export default observer(function Create({carModelStore,carMakeStore}) {
   ...
   const createCarModel = () => {
    if(carMakeId.current.value === 'null' || carModelName.current.value === '' || carModelAbrv.current.value === '') {
        setShow(true) 
        setType({
            style : 'danger',
    ...
    carModelStore.createCarModel(carModelName.current.value,carModelAbrv.current.value,carMakeId.current.value)
    setShow(true) 
    setType({
        style : 'success',
        title : 'Success',
   ...
   const createCarMake = () => {
    ...
    carMakeStore.createCarMake(carMakeName.current.value,carMakeAbrv.current.value)
    setShow(true) 
    setType({
        style : 'success',
        title : 'Success',
  ...
  return (
    <>
    <div className='d-flex justify-content-center align-items-center bg-light' style={{height:"100vh",width:"100vw",position:"relative"}}>
        <Form className='w-75 bg-light' style={{height:"100%",position:"relative"}}>
            <Form.Group className='p-4 w-75 mx-auto' >
                ...
                <Form.Label className="mt-2" required>Car make</Form.Label>
                <div className='d-flex' style={{gap:"10px"}}>
                    ...
                    {carMakeStore.carMakeList.map(carMake => {
                        return <option key={carMake.id} value={carMake.id}>{carMake.name}</option>
                    })}
                    </Form.Select>
                    <Button style={{width:"20%"}} variant='outline-primary' onClick={() => {setModalShow(true)}}>Edit list</Button>
                </div>
            </Form.Group>
            <Button style={{position:"absolute",left:"50%",transform:"translateX(-50%)"}} onClick={() => {createCarModel()}}>Submit New Car</Button>
            ...
            <Button style={{position:"absolute",left:"50%",transform:"translateX(-50%)"}} onClick={() => {createCarMake()}}>Submit New Car Make</Button>
        </Form>
        <Button variant='outline-primary' onClick={() =>{navigate("/")}} style={{top:"10px",right:"10px",position:"absolute"}}>X</Button>
